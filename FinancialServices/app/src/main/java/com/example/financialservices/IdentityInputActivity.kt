...
import com.example.financialservices.databinding.ActivityIdentityInputBinding
import com.example.financialservices.util.ViewUtil.hideKeyboard
import com.example.financialservices.util.ViewUtil.setOnEditorActionListener
import com.example.financialservices.util.ViewUtil.showKeyboard
import com.example.financialservices.util.ViewUtil.showKeyboardDelay
...
class IdentityInputActivity : AppCompatActivity() {
    ...
    private fun initView(){
        with(binding){
            nameEdit.setOnEditorActionListener(EditorInfo.IME_ACTION_NEXT){
                ...
                if(validName()) {
                    ...
                    if(phoneLayout.isVisible){
                        ...
                        birtydayEdit.showKeyboard()
                    }
                } else{
                    confirmButton.isVisible = false
                    nameLayout.error = "1자 이상의 한글을 입려해주세요."
                }
            }

            birtydayEdit.doAfterTextChanged {
                if(birtydayEdit.length() > 7){
                    if(validBirthday()){
            ...
            birtydayEdit.setOnEditorActionListener(EditorInfo.IME_ACTION_DONE){
                val isValid = validBirthday() && birtydayEdit.length() > 7
                if(isValid){
    ...
    fun onClickDone(){
        if(!validName()){
            binding.nameLayout.error = "1자 이상의 한글을 입려해주세요."
            return
        }
    ...
    private fun validBirthday() = !binding.birtydayEdit.text.isNullOrBlank()
            && REGEX_BIRTHDAY.toRegex().matches(binding.birtydayEdit.text!!)
    ...
    companion object{
        ...
        private const val REGEX_BIRTHDAY = "^(19|20)[0-9]{2}(0[1-9]|1[0-2])(0[1-9]|[12][09]|3[01])"
